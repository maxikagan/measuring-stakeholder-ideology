...
This report summarizes the **Consumer Edge "Brand and Geography Cohort Breakout" (Combined Panels 1+2)** dataset, a consumer spending panel covering credit/debit card transactions aggregated by brand, geography, and time period. The data spans Q4 2022 through Q2 2025 with ~208M rows in a consolidated parquet file.
...
```{r load-data}
parquet_path <- "/global/scratch/users/maxkagan/01_foot_traffic_location/consumer_edge_combined_2026-01-12/parquet/consumer_edge_brand_geo_combined.parquet"

dt <- as.data.table(read_parquet(parquet_path))

n_rows_sample <- nrow(dt)
n_cols <- ncol(dt)
file_size_gb <- round(file.info(parquet_path)$size / 1e9, 2)
```

**Data Source:** `r parquet_path`

- **File size:** `r file_size_gb` GB (parquet format)
...
1. **Brand matching:** Build crosswalk to Advan brands and Politics at Work employers
2. **Panel construction:** Create brand × month panel for regression analysis
3. **Event study:** Design analysis around 2024 election as salience shock
...
```{r data-source}
data.frame(
  Property = c("File", "Format", "Size", "Rows", "Columns"),
  Value = c(
    basename(parquet_path),
    "Apache Parquet",
    paste0(file_size_gb, " GB"),
    format(n_rows_sample, big.mark = ","),
    n_cols
  )
) %>%
  kable(caption = "Data Source Details") %>%
  kable_styling(bootstrap_options = c("striped", "hover"), full_width = FALSE)
```
...
*Data source: Consumer Edge "Brand and Geography Cohort Breakout" (Combined 1+2) via Dewey Data (parquet format)*
